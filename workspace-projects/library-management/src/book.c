...
uint8_t add_book() {
	...
	book[book_count].publication_date.date = get_input_date(book[book_count].publication_date.month);
	while ((ch = getchar()) != '\n' && ch != EOF);
	...
	book[book_count].indatabase_date.date = get_input_date(book[book_count].indatabase_date.month);
	while ((ch = getchar()) != '\n' && ch != EOF);
